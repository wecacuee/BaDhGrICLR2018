...
The top row shows the top view of the robot moving through the maze along with goal location in orange, second row shows the first person view which is the only input available to the agent (except reward).}
...
Our experiments do not show any evidence of mapping in the cases when algorithms are evaluated on unseen maps and no evidence of optimal path planning even in cases when the map is same and only goal is randomized.
