...
One such algorithm by \cite{MiPaViICLR2017}, has shown promise in exploring and finding the goal efficiently within complex environments. Notably, this is done using monocular first-person view only.
...
\cite{MiPaViICLR2017} train and test their algorithms with randomized goals, randomized spawns but constant map structures and show that algorithm is able to exploit the knowledge of the goal location at evaluation time to maximize reward.
...
We expand on the analysis in \cite{MiPaViICLR2017} to address its limitations and ask the question whether DRL-based algorithms such as \NavAiiiCDiDiiL{} perform any mapping followed by shortest path planning.
