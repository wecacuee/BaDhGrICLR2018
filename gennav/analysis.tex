...
The results for training on $N$ maps, where $N \in \{10, 100, 500, 1000\}$, and testing on 100 unseen maps are shown in Fig~\ref{fig:num-training-maps}. We observe that their is significant jump of average reward and averge goal hits when number of training maps is increased from 10 to 100 but no significant increase when number of training maps are increased from 100 to 500 to 1000.
...
\paragraph{Wrench map and Goal map}
...
We observe that the attention is uniformly distributed on the image when the agent spawns. The attention narrows down few pixels in the center when the agent is navigating through the corridor. It spreads to the entire image around turns and junctions. The attention also pays close attention to important objects like goal, apples and unique decals.
