...
The metric \emph{distance traveled : shortest path} is defined as the ratio of total distance traveled by the agent verses the sum of approximate shortest distances to the goal for every goal hit, except for the first iteration of finding the goal.
We ignore the first iteration of finding the goal, considering it as the exploration time and not penalizing the agent for taking a longer than shortest path during exploration.
Also note, that the shortest distance between the spawn point and goal location is computed in the block world and hence is only an approximation of the shortest distance.


Following \cite{MiPaViICLR2017}, we report \emph{\LatencyOneGtOne{}}, a ratio of time taken to hit the goal for the first time (exploration time) verses the average amount of time taken to hit goal subsequently (exploitation time).
